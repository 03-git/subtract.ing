...
TMPL
        ...
        [0-9]|[0-9][0-9])
            ...
            ;;
        "ask sub "*)
            __subtract_handle "ask llama.cpp ${input#ask sub }"
            return
            ;;
        "ask add "*)
            __subtract_handle "ask curl ${input#ask add }"
            return
            ;;
        "ask local "*)
    ...
    if [ -z "$cmd" ] && [ -z "$(__subtract_get_mode)" ]; then
        local man_cmd
...
# DISABLED:     # T2: local generative model (if available)
# DISABLED:     if [ -z "$cmd" ]; then
# DISABLED:         cmd=$(__subtract_generate "$input")
# DISABLED:         if [ -n "$cmd" ]; then
# DISABLED:             tier="T2"
# DISABLED:             tag="stdout"
# DISABLED:         fi
# DISABLED:     fi
# DISABLED: 
# DISABLED:     # T4: cloud escalation (if configured)
# DISABLED:     if [ -z "$cmd" ]; then
# DISABLED:         cmd=$(__subtract_cloud "$input")
# DISABLED:         if [ -n "$cmd" ]; then
# DISABLED:             tier="T4"
# DISABLED:             tag="stdout"
# DISABLED:         fi
# DISABLED:     fi
    ...
    if [ -n "$cmd" ] && [ "$cmd" != "null" ]; then
        ...
        __subtract_route_by_mode "$input"
    fi
}

# --- r/w/x session mode ---

__subtract_session_file() {
    local tty_id
    tty_id=$(tty 2>/dev/null | tr "/" "_")
    [ "$tty_id" = "not_a_tty" ] && tty_id="notty_$$"
    echo "$SUBTRACT_DIR/session${tty_id}"
}

__subtract_get_mode() {
    local session
    session=$(__subtract_session_file)
    cat "${session}.mode" 2>/dev/null
}
...
__subtract_prompt_mode() {
    local mode
    printf "(r)eading (w)riting e(x)ecution? " > /dev/tty
    if [[ -n "$ZSH_VERSION" ]]; then read -k1 mode; else read -n1 mode; fi < /dev/tty
    echo > /dev/tty
    case "$mode" in
        r|R) mode="r" ;;
        w|W) mode="w" ;;
...
__subtract_read() {
    local input="$*"
    local session context result
    session=$(__subtract_session_file)

    [ -f "${session}.log" ] && context=$(tail -n 30 "${session}.log")
    # Fallback to Claude session context if no handler context
    [ -z "$context" ] && context=$(__subtract_claude_context)
    local prompt
    if [ -n "$context" ]; then
...
__subtract_route_by_mode() {
    local input="$*"
    local mode

    # Check for mode switch command
    case "$input" in
        "mode r"|"mode R") __subtract_set_mode "r"; echo "[mode: reading]"; return 0 ;;
        "mode w"|"mode W") __subtract_set_mode "w"; echo "[mode: writing]"; return 0 ;;
        "mode x"|"mode X") __subtract_set_mode "x"; echo "[mode: execution]"; return 0 ;;
    ...
    case "$mode" in
        r) __subtract_read "$input" ;;
        w) __subtract_write "$input" ;;
        x)
            local cmd
            cmd=$(__subtract_exec "$input")
            if [ -n "$cmd" ] && [ "$cmd" != "null" ]; then
                echo "[x] $cmd"
                printf "[enter/n] " > /dev/tty
...
# --- interrogative aliases: shadow vestigial commands for natural input ---
__subtract_passthrough() { __subtract_route_by_mode "$*"; }

alias what="__subtract_passthrough what"
alias where="__subtract_passthrough where"
alias who="__subtract_passthrough who"
